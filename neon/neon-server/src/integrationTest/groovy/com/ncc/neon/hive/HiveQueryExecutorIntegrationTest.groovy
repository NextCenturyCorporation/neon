...
class HiveQueryExecutorIntegrationTest {
    ...
    private static final JdbcClient jdbcClient = new JdbcClient("org.apache.hadoop.hive.jdbc.HiveDriver", "hive", "default", "localhost:10000")
...
//            println(iterator.next().getFieldValue("donor_lastname"))
//        }
//
//        println(result.toJson())
    ...
    private static void insertData() {
        ...
        jdbcClient.executeQuery("create database ${DATASOURCE_NAME}")
        jdbcClient.executeQuery(testTableDdlFile.text)
        jdbcClient.executeQuery("load data local inpath '${testTableTempDataFile.absolutePath}' overwrite into table ${DATASOURCE_NAME}.${DATASET_ID}")
    }

    private static void deleteData() {
        jdbcClient.executeQuery("drop table ${DATASOURCE_NAME}.${DATASET_ID}")
        jdbcClient.executeQuery("drop database ${DATASOURCE_NAME}")
    }
