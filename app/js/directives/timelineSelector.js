...
angular.module('timelineSelectorDirective', []).directive('timelineSelector', ['ConnectionService',
	function (connectionService) {
		...
		return {
			templateUrl: 'app/partials/timelineSelector.html',
			restrict: 'EA',
			scope: {
			...
			link: function ($scope, element, attr) {
				...
				$scope.getMinMaxDates = function (success) {
					...
					var  minDateQuery = new neon.query.Query()
						.selectFrom($scope.databaseName, $scope.tableName)
						.where($scope.dateField, '!=', null).aggregate(neon.query.MIN, $scope.dateField, "minDate");
					...
					connectionService.getActiveConnection().executeQuery(minDateQuery, function (queryResults) {
						if (queryResults.data.length > 0) {
							...
							$scope.referenceStartDate = new Date(queryResults.data[0].minDate);
					...
					/*var maxDateQuery = new neon.query.Query()
						.selectFrom($scope.databaseName, $scope.tableName)
						.where($scope.dateField, '!=', null).sortBy($scope.dateField, neon.query.DESCENDING)
						.groupBy($scope.dateField).limit(1);*/
					var  maxDateQuery = new neon.query.Query()
						.selectFrom($scope.databaseName, $scope.tableName)
						.where($scope.dateField, '!=', null).aggregate(neon.query.MAX, $scope.dateField, "maxDate");
					...
					connectionService.getActiveConnection().executeQuery(maxDateQuery, function (queryResults) {
						if (queryResults.data.length > 0) {
							...
							console.log(new Date(queryResults.data[0]["_id"][$scope.dateField]));
							console.log(queryResults.data[0]["_id"][$scope.dateField]);
							console.log(queryResults.data[0]["_id"]);
							console.log(queryResults.data[0]);


							$scope.referenceEndDate = new Date(queryResults.data[0].maxDate);
				...
				$scope.createTimelineData = function (queryResults) {
					...
					var a = Math.abs(endDate - startDate);
					...
					for (i = 0; i < rawLength; i++) {
						...
						var a = resultDate - startDate;
						var b = Math.abs(resultDate - startDate);
						var c = Math.floor(Math.abs(resultDate - startDate) / $scope.millisMultiplier);
						var d = queryData[Math.floor(Math.abs(resultDate - startDate) / $scope.millisMultiplier)]
