...
import com.ncc.neon.query.elasticsearch.ElasticSearchRestConversionStrategy

/**
 Tests the ElasticSearchConversionStrategy
 */
class ElasticSearchRestConvertQueryTest extends AbstractConversionTest {
    ...
    protected def doConvertQuery(query, queryOptions) {
        ElasticSearchConversionStrategy conversionStrategy = new ElasticSearchConversionStrategy(
                filterState: filterState, selectionState: selectionState)
        return conversionStrategy.convertQuery(query, queryOptions)
    }

    private XContentParser getParserForQuery(query) {

        System.out.println(StackTraceUtils.sanitize(new Throwable()).stackTrace[1].methodName)

        def source = query.source
    ...
    void assertQueryWithWhereClause(query) {
        XContentParser parser = getParserForQuery(query)
        def map = parser.map()
    ...
    protected void assertQueryWithOffsetClause(query) {
        def parser = getParserForQuery(query)
        def map = parser.map()
