...
class ArrayCountQueryWorker extends AbstractMongoQueryWorker {
    ...
    MongoQuery createArrayCountQuery(MongoQuery mongoQuery, String field, int limit, FilterState filterState, SelectionState selectionState, boolean isFieldArray) {
    ...
    QueryResult executeQuery(MongoQuery mongoQuery) {
        ...
        while (results.hasNext()) {
            ...
            // Don't convert the keys to strings because users wouldn't be able to set filters on the keys (since they don't match the values in the database).
            def key = row.get('_id')
